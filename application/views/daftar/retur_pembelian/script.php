...
<script>
    $(document).ready(function () {
        ...
        $('#id_penerimaan').change(function () {
            ...
            if (id_penerimaan) {
                $.ajax({
                    url: '<?php echo site_url('aktifitas/retur_pembelian/get_detail_penerimaan'); ?>',
                    method: 'POST',
                    data: {
                    ...
                    success: function (response) {
                        ...
                        if (response.length > 0) {
                            // Set supplier dari data pertama (semua barang dalam satu penerimaan memiliki supplier yang sama)
                            $('#id_supplier').val(response[0].nama_supplier);
                            $('#id_supplier_hidden').val(response[0].id_supplier);
                            ...
                            $.each(response, function (index, item) {
                                ...
                                var html = '<tr>' +
                                    ...
                                    '</td>' +
                                    '<td>' +
                                    '<input type="text" class="form-control" value="' + item.jumlah_diterima + ' ' + item.satuan + '" readonly>' +
                                    '</td>' +
                                    '<td><input type="number" class="form-control" name="jumlah_retur[]" min="1" max="' + item.jumlah_diterima + '" required placeholder="Jumlah retur"></td>' +
                            ...
                            var html = '<tr>' +
                                ...
                                '<em>Tidak ada barang dalam penerimaan ini</em>' +
                                '</td>' +
                                '</tr>';
        ...
        $('form').on('submit', function (e) {
            ...
            if (!$('#id_penerimaan').val()) {
                e.preventDefault();
                alert('Harap pilih penerimaan barang terlebih dahulu!');
                return false;
            }
